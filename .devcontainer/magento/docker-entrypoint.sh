...
if [[ "$1" == apache2* ]] || [ "$1" == php-fpm ]; then
        ...
        if [ ! -e index.php ] && [ ! -e libraries/src/Version.php ]; then
                # if the directory exists and OpenMage doesn't appear to be installed AND the permissions of it are root:root, let's chown it (likely a Docker-created directory)
